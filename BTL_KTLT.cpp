...
stringArr divString(string line){
...
stringArr divStringSpace(string str){
	stringArr temp; 
	temp.count = 0;
	int len = str.length();
	int j = 0, i = 0;
...
//  nhap vao 1 xau ky tu, tim cac tu trong xau ky tu nay trong cac truong cua tu, dua ra tu neu tu co chua 75% so luong cac tu trong xau input
		...
		if(result == -1){
			...
			if(line == "2") {
				...
				if(line == ""){
					...
					stringArr temp = divStringSpace(line);
					int count = 0;
					cout << "\n                            ------------------KET QUA--------------------";
					for(int i = 0; i < word::count; i++){
...
void editType(){
	...
	for(int i= 0; i< tabletype[result].fieldcount;i++) {
        ...
        cout << "\n\n               Chuc nang da chon: Sua the loai. \n\n";
	    printTypeName(result);
		cout << "               Ban muon sua Ten Truong hoac xoa truong \""<< tabletype[result].fieldName[i]<< "\", hoac xoa truong nay di: c/k/x? ";
		getline(cin,line);
		if(line == "k") continue;
		else if(line == "c"){
		        do{ 
		        ...
		        time = time + (float)(finish-start)/CLOCKS_PER_SEC; 
		     	      ...
		     	      for(int k = i; k <tabletype[result].fieldcount-1;k++){
		     	      	  tabletype[result].fieldName[k] = tabletype[result].fieldName[k+1];
					   }
		     	      tabletype[result].fieldcount--; 
		     	      i--;
		     	      int code = tabletype[result].typeCode;
		     	      for(int j = 0; j < word::count; j++){
		     	      	if(words[j].typeCode == code){
		     	      		for(int k = i; k < words[j].valuesCount-1; k++){
		     	      			words[j].values[k] = words[j].values[k+1]; 
							   }
							   words[j].valuesCount--;
						   }
					   }
	...
	do{
        ...
        cout << "\n\n               Chuc nang da chon: Sua the loai. \n\n";
	    printTypeName(result);
	    cout << "               Ban co muon them truong khong. <c/k>. ";
	    getline(cin,line); 
	    line = checkInput(line);
	    string line1;
		if(line == "c") {
		     ...
		     tabletype[result].fieldName[tabletype[result].fieldcount] = line1;
			 tabletype[result].fieldcount++;
			 for(int i = 0; i < word::count; i++){
			 	if(words[i].typeCode = tabletype[result].typeCode){
			 		words[i].values[words[i].valuesCount] = "Chua co thong tin";
			 		words[i].valuesCount++;
				 }
			 }
		}
